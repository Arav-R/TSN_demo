...
bool Coap::register_observer(uint8_t* token, uint8_t tokenlen, IPAddress ip, int port, String url) {
    if (observer_len < COAP_MAX_OBSERVERS) {
        CoapObserver new_ob;
        new_ob.token = token;
        new_ob.tokenlen = tokenlen;
        new_ob.observer_ip = ip;
        new_ob.observer_port = port;
        new_ob.server_url = url;
        observers[observer_len] = new_ob;
        observer_len++;
        return true; 
    } 
    else {
...
int Coap::loop() {
    ...
    bool register_notification = false;
    String url = "";
    bool ret = false;
    while (packetlen > 0) {
        ...
        if (COAP_HEADER_SIZE + packet.tokenlen < packetlen) {
            ...
            // add support for observer(rfc7641)
            for (int i = 0; i < packet.optionnum; i++) {
                if (packet.options[i].number == COAP_OBSERVE) {
                    // parse registartion request
                    if (packet.options[i].length == 0) {
                        // this indicate a registration request
                        register_notification = true;
                    }
                    else {
                        register_notification = false;
                    }
                }
            }
        ...
        if (packet.type == COAP_ACK) {
            ...
            resp(packet, url, _udp->remoteIP(), _udp->remotePort());
            ...
            if (!uri.find(url)) {
                ...
                uri.find(url)(packet, url, _udp->remoteIP(), _udp->remotePort());
                sendPacket(packet, _udp->remoteIP(), int(_udp->remotePort()));
        ...
        // add any registry requests
        if (register_notification) {
            ret = register_observer(packet.token, packet.tokenlen, _udp->remoteIP(), _udp->remotePort(), url);
        }
        // next packet
        packetlen = _udp->parsePacket();
    }
    
    // Now notify any registered observers
    for (int i = 0; i < observer_len; i++) {
        CoapPacket packet;
        // packet.type = COAP_ACK;
        // packet.code = COAP_CONTENT;
        packet.token = observers[i].token;
        packet.tokenlen = observers[i].tokenlen;
        // packet.payload = ;
        // packet.payloadlen = ;
        //packet.messageid = ;
        //packet.optionnum = ;
        String observer_seq_str = String(observers[i].get_observe_num());
        char opt_payload[observer_seq_str.length() + 1];
        observer_seq_str.toCharArray(opt_payload, observer_seq_str.length() + 1);
        packet.addOption(COAP_OBSERVE, observer_seq_str.length(), (uint8_t *)opt_payload);
        uri.find(observers[i].server_url)(packet, url, observers[i].observer_ip, observers[i].observer_port);
        sendPacket(packet, observers[i].observer_ip, observers[i].observer_port);
    }
...
String Coap::print_shit() {
    String uri_debug = uri.print_shit();
    String observer_debug = "observer reg: \n";
    for (int i = 0; i < observer_len; i++) {
        observer_debug.concat(" ");
        observer_debug.concat(i);
        observer_debug.concat(" ");
        observer_debug.concat(observers[i].observer_ip);
        observer_debug.concat(observers[i].server_url);
    } 

    return uri_debug.concat(observer_debug);
}